...
def modify_tif(
    input_tif: str,
    SA_SQ_KM_FROM_SHAPEFILE_constant: float,
    ...
    # create new bands
    SA_SQ_KM_band = np.full_like(
        raster_data[0], SA_SQ_KM_FROM_SHAPEFILE_constant, dtype=raster_data.dtype
    )
    ...
    with rasterio.open(modified_tif, "w", **profile) as dst:
        ...
        bands_to_fill = 9 - 5
        for i in range(
            raster_data.shape[0] + 1, raster_data.shape[0] + 1 + bands_to_fill
        ):
            # print(f"Writing null band... at ({i})")
            null_band = np.full_like(
                raster_data[0],
                model_data.NAN_SUBSTITUTE_CONSANT,
...
def predict(input_tif: str, lakeid: int, tags, display=True):
    ...
    non_finite_val_mask = ~np.isfinite(
        raster_data[0]
    )  # if first band at that pixel is nan, inf, or -inf, usually rest are too (helps remove "garbage" val from output later)

    raster_data_2d[~np.isfinite(raster_data_2d)] = (
        model_data.NAN_SUBSTITUTE_CONSANT
    )  # Replace with NAN_SUB_CONSTANT or mean of general, but this pixels output  will later be removed anyway
    ...
    if not IS_IN_PRODUCTION_MODE:
        ...
        output_tif_csv = (
            add_suffix_to_filename_at_tif_path(input_tif, "predicted") + ".csv"
        )
    ...
    predictions_raster[non_finite_val_mask] = (
        np.nan
    )  # if the input value was originally nan, -inf, or, ignore its (normal-seeming) output and make it nan
    ...
    if display:
        ...
        max_cbar_value = 60
        plt.imshow(
            predictions_raster, cmap="viridis", vmin=min_cbar_value, vmax=max_cbar_value
        )
        plt.colorbar()
...
for path_tif in tqdm(paths):
    ...
    try:
        ...
        # Get constants
        SA_SQ_KM_constant, pct_dev_constant, pct_ag_constant = model_data.get_constants(
            id
        )
        # print(f"Constants based on id({id}): ", SA_SQ_KM_constant, pct_dev_constant, pct_ag_constant)

        modified_path_tif = modify_tif(
            path_tif, SA_SQ_KM_constant, pct_dev_constant, pct_ag_constant
        )

        output_tif, predictions_loop = predict(
            path_tif, id, tags, display=VISUALIZE_PREDICTIONS
        )

        if VISUALIZE_PREDICTIONS:
            print("Output tif: ", os.path.join(os.getcwd(), output_tif))

        output_path_png = save_png(
            path_tif, png_out_folder, predictions_loop, date, scale, display=False
        )

        if IS_IN_PRODUCTION_MODE:
            upload_spatial_map(
                id, output_tif, output_path_png, date, corners, scale, image_index, satellite
            )

        with open(
            os.path.join(session_statues_path, f"successes_{session_uuid}.status.txt"),
            "a",
        ) as file_obj:
...
with open(
    os.path.join(session_statues_path, f"error_paths_{session_uuid}.json"), "w"
) as file:
