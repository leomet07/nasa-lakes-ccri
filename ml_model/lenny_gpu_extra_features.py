...
input_tif = '/content/drive/MyDrive/NASA/lenny_google_collab/13_bands_chautauqua.tif'
...
SA_constant = 13000
Max_depth_constant = 78
pct_dev_constant = 8.44
pct_ag_constant = 24.88
...
# output GeoTIFF file
output_tif = '13_bands_chautauqua_modified.tif'
...
def prepare_data(df_path, lagosid_path, lulc_path, random_state=621, test_size=0.1):
    ...
    df = df[['chl_a', '443', '493', '560', '665','703', '740', '780', '834', '864','SA','Max.depth','pct_dev','pct_ag']]
    df = df.fillna(-99999)
...
lagosid_path = "/content/drive/MyDrive/NASA/lenny_google_collab/ccri_lakes_withLagosID.csv"
...
data = data[data['chl_a'] < 350] # most values are 0-100, remove the crazy 4,000 outlier
# print(data.head)
...
known_fast_model = RandomForestRegressor(**oparams) #2 for mse
...
import joblib
joblib.dump(known_fast_model, "test.joblib")

test_load_model = joblib.load("test.joblib") ## Works!

import matplotlib.pyplot as plt
import rasterio
input_tif = '13_bands_chautauqua_modified.tif'
...
plt.axis('off')
cbar = plt.colorbar()
cbar.set_label('Predicted chlorophyll-A in ug^3/L')

cbar.set_ticks(values)
cbar.set_ticklabels([str(val) for val in values])

# save the png
output_png = 'Chau_2019_gpu_new.png'
plt.savefig(output_png, format='png', bbox_inches='tight', pad_inches=0, transparent=True)
plt.show()
...
{'split_criterion': 2, 'n_estimators': 200, 'min_samples_split': 7, 'min_samples_leaf': 3, 'max_features': 1.0, 'max_depth': 30}
'''
